...
# Loads data from the json file in the dir folder
def load_game(filename="gamesave.json"):
    ...
    if not os.path.exists(filepath):
        ...
        slow_text_centered("Loading you into a new game file", min_delay=0.02, max_delay=0.08, newLine=True,
...
# Adds buffs onto player
def add_buff(buff):
    for stat, value in buff.items():
        player[stat] += value

# Removes stats onto player
...
def combat_system(enemy, fight_choice, enemy_name):
    ...
    def health_check():
        if temporary_player_health <= 0:
            clear_screen()
            slow_text_centered("You have died. Please restart your game and use your game save", min_delay=0.02,
                               max_delay=0.08, newLine=True, vertical_padding=True)
            close()

    if enemy["health"] == 0:
        clear_screen()
        slow_text_centered(f"You have killed {enemy}!", min_delay=0.02, max_delay=0.08, newLine=True,
    ...
    while True:
        if fight_choice == "Attack":
            ...
            if temporary_mana < 20:
                slow_text_centered("You don't have enough mana it requires 20", min_delay=0.02, max_delay=0.08,
                                   newLine=True, vertical_padding=True)
                time.sleep(3)
                clear_screen()
    ...
    if temporary_player_health >= 0 and temporary_enemy_health >= 0:
        ...
        clear_screen()
        slow_text_centered(f"{enemy_name} dealt {temporary_enemy_damage} to you!", min_delay=0.02, max_delay=0.08,
                           newLine=True, vertical_padding=True)
        time.sleep(3)
        clear_screen()
        if temporary_mana >= 0:
            slow_text_centered(f"You currently have {temporary_player_health} health left!", min_delay=0.02,
                               max_delay=0.08, newLine=True, vertical_padding=False)
            clear_screen()
        else:
            slow_text_centered(f"You currently have 0 health left!", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            clear_screen()
        health_check()
        time.sleep(3)
...
def setup():
    global invalid_attempts, name, user_class, player, xp, level, p_bal, basic, speedbuff_applied, classes, weapon
    clear_screen()
    ...
    slow_text_centered("Your device activity shows that your into very intensive fights", newLine=True,
                       vertical_padding=False)
    slow_text_centered("Don't worry in Hyperia we have quite the challengers for you", newLine=True,
                       vertical_padding=False)
    ...
    # Class Prints
    clear_screen()
    slow_text_centered("These are your choices below", min_delay=0.02, max_delay=0.08, newLine=True,
                       vertical_padding=True)
    slow_text_centered("Beserker, a combat heavy class with fast attacks, though you will not be wise...",
                       min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
    slow_text_centered(
        "Mage, filled with magic and knowledge. Mana overflows from you. You will be cursed with a short life...",
        min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
    slow_text_centered(
        "Tank, nothing can stop you. You have a heart made of iron and a body made of steel. Though your quite slow...",
        min_delay=0.02, max_delay=0.08, vertical_padding=False)
    slow_text_centered(
        f"ERROR 404: CLASS NOT AVAILABLE FOR /-/=-/ BEEP {name} you must think out the box for this class",
        min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
    slow_text_centered(f"So {name} what will be your choice?", min_delay=0.02, max_delay=0.08, newLine=True,
                       vertical_padding=False)
    ...
    while True:
        ...
        if player not in ["Mage", "Tank", "GM", "Beserker"]:
            ...
            if invalid_attempts > MAX_INVALID_ATTEMPTS:
                clear_screen()
                slow_text_centered("I'm not that forgiving restart your game to retry", newLine=True,
                                   vertical_padding=True)
                close()
        else:
            reset_count()
    ...
    while True:
        ...
        if player == 'GM':
            ...
            if encrypt == 'axinesky':
                clear_screen()
                slow_text_centered("You have obtained the GM class", min_delay=0.02, max_delay=0.08, newLine=True,
                                   vertical_padding=True)
                reset_count()
                break
            else:
                clear_screen()
                slow_text_centered("Wrong, restart your game to select a new class disappointing..", min_delay=0.02,
                                   max_delay=0.08, newLine=True, vertical_padding=False)
            close()
            break
        else:
    ...
    clear_screen()
    slow_text_centered("Now you chose your class lets actually spawn your character shall we?", min_delay=0.02,
                       max_delay=0.08, newLine=False, vertical_padding=True)
    clear_screen()
    ...
    slow_text_centered("Whoops seems like i spawned in a boss in the Hub LOL", min_delay=0.02, max_delay=0.12,
                       newLine=True, vertical_padding=True)
    slow_text_centered("Looks like your gonna have to fight it im not gonna rollback my git commit xd", min_delay=0.02,
                       max_delay=0.12, newLine=True, vertical_padding=False)
    ...
    while hub_boss_stats["health"] > 0:
        ...
        if temporary_enemy_health < 0:  # Kill check
            clear_screen()
            slow_text_centered(f"You have killed {hub_boss}!", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=True)
            health_refresh()
            ...
            if user_class == "Mage":
                ...
                weapon = "Ban Hammer"
            slow_text_centered(f"You have obtained a {weapon} your base stats have been upgraded!", min_delay=0.02,
                               max_delay=0.08, newLine=True, vertical_padding=False)
            ...
            if user_class == "Mage":
                ...
                clear_screen()
                slow_text_centered("How did you even get here with no class?? ur banned", min_delay=0.02,
                                   max_delay=0.08, newLine=True, vertical_padding=True)
                close()
            for key, value in basic.items():
                slow_text_centered(f"You have gained {key} buff of {value}", min_delay=0.02, max_delay=0.08,
                                   newLine=True, vertical_padding=False)
            time.sleep(3.5)
            add_buff(basic)
            clear_screen()
            slow_text_centered("Well then i guess my attempt for you to quit didn't work", min_delay=0.02,
                               max_delay=0.08, newLine=True, vertical_padding=True)
            slow_text_centered("Alright fine ill send you to the Hub", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            update_location("Hub")
            save_game()
...
def menu_screen():
    ...
    while True:
        if choice == 'Profile':
            ...
            break
        if choice == "Location":
            slow_text_centered(f"You are currently saved at {location}", min_delay=0.04, max_delay=0.12, newLine=True, vertical_padding=True)
            time.sleep(3)
            menu_screen()
            break
...
slow_text_centered("This is an island with infinite problems with solutions only you can solve!", newLine=True,
                   vertical_padding=False)
...
# Globals for outer module warning
global hub_npc_choice, blacksmith_screen, npc_screen, guild_screen, dwarven_screen


def hub():
    global hub_npc_choice, p_bal, blacksmith_screen, npc_screen, guild_screen, dwarven_screen
    while True:
        def hub_screen():
            clear_screen()
            slow_text_centered("Welcome to the Hub", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=True)
            slow_text_centered("You can interact with the options below", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("Speak to locals", min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
            slow_text_centered("Go to the blacksmith", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("Enter the Dwarven Mines", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("Enter the Guild Hall", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("Menu", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)

            screen_choice = input("> ").lower().strip()
            return screen_choice

        hub_choice = hub_screen()

        if hub_choice == 'go to the blacksmith':
            blacksmith_screen()
        ...
        def npc_screen():
            ...
            if hub_npc_choice == "speak to the certi":
                ...
                if hub_npc_choice == "no":
                    hub_screen()
            if hub_npc_choice == "speak to axinesky":
                clear_screen()
                slow_text_centered(f"Whats up {name} im basically the creator", min_delay=0.02, max_delay=0.08,
                                   newLine=True, vertical_padding=True)
                slow_text_centered(f"If you haven't spoke to Certi you probably should", min_delay=0.02, max_delay=0.08,
                                   newLine=True, vertical_padding=False)
                slow_text_centered(f"You should really explore the Hub and then head to Dwarven Mines", min_delay=0.02,
                                   max_delay=0.08, newLine=True, vertical_padding=False)
                slow_text_centered(
                    f"By the way the Guild Hall will be VERY handy after you go there speak to Maven in the Blacksmith",
                    min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
                slow_text_centered(f"Before you leave im the AES256 if you know what i mean hahaha", min_delay=0.02,
                                   max_delay=0.08, newLine=True, vertical_padding=False)
            ...
            if hub_npc_choice == "speak to damian":
                ...
                if hub_npc_choice == "no":
                    hub_screen()
        # Function for Blacksmith Screen
        def blacksmith_screen():
            global p_bal

            clear_screen()
            slow_text_centered("Hey my name is Maven", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=True)
            slow_text_centered("I can upgrade your weapon!", min_delay=0.02,
                               max_delay=0.08, newLine=True, vertical_padding=False)
            slow_text_centered("My prices are a bit high but you can make some money off the guild or..",
                               min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
            slow_text_centered("You can speak to Damian but if anyone asks you wasn't here", min_delay=0.02,
                               max_delay=0.08, newLine=True, vertical_padding=False)
            slow_text_centered("Anyway what can i do for you", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("\n Upgrade your weapon", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("\n Buy a weapon", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            blacksmith_choice = input("> ").lower().strip()
            if blacksmith_choice == "upgrade your weapon":
                clear_screen()
                blacksmith_upgrade = {"mana": 5, "attack_speed": 5, "damage": 5}
                if weapon == "Basic Sword":
                    if p_bal > 50:
                        slow_text_centered("Your Weapon has been upgraded", min_delay=0.02, max_delay=0.08,
                                           newLine=True, vertical_padding=False)
                        slow_text_centered("You have been charged 50$", min_delay=0.02, max_delay=0.08,
                                           newLine=True, vertical_padding=False)
                        add_buff(blacksmith_upgrade)
                        p_bal -= 50
                        time.sleep(2)
                        hub_screen()
                elif weapon == "Basic Shield":
                    if p_bal > 50:
                        slow_text_centered("Your Weapon has been upgraded", min_delay=0.02, max_delay=0.08,
                                           newLine=True, vertical_padding=False)
                        slow_text_centered("You have been charged 50$", min_delay=0.02, max_delay=0.08,
                    ...
                    if p_bal > 50:
                        ...
                        time.sleep(2)
                        hub_screen()
        # Function for Guild Screen (main progression)
        def guild_screen():
            global xp, p_bal
            slow_text_centered("Hey welcome to the Guild", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("We have a quest open to fight some goblins", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("There are 5 goblins in this quest", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            slow_text_centered("Would you like to fight them?", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
            guild_choice = input("> ").lower().strip()
            if guild_choice == "yes":
                    for i in range(5):
                    # Same concept used but a for loop involved
                        goblin = list(hostile.keys())[1]
                        goblin_stats = copy.deepcopy(hostile["Goblin"])

                        slow_text_centered(f"You are being attacked by a {goblin}", min_delay=0.02, max_delay=0.12, newLine=True,
                                           vertical_padding=True)
                        clear_screen()
                        time.sleep(1.5)
                        while goblin_stats["health"] > 0:
                            ...
                            if temporary_enemy_health < 0:  # Kill check
                                ...
                                slow_text_centered(f"You have killed {goblin}!", min_delay=0.02, max_delay=0.08, newLine=True,
                                                   vertical_padding=True)
                                health_refresh()
                                slow_text_centered(f"You gained 10 XP", min_delay=0.02, max_delay=0.08, newLine=True, vertical_padding=False)
                                xp += 10
        ...
        def dwarven_screen():
            if xp < 10000:
                clear_screen()
                slow_text_centered("Maybe come back after you gained 10000 XP")
                time.sleep(2)
                hub_screen()
            else:
                slow_text_centered("Wow you made it here.", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
                slow_text_centered("You have made it to the end for now your stats must be crazy", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
                slow_text_centered("Even though this is a school project i'll keep updating it", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
                slow_text_centered("In the future maybe ill recode it with pygame and post it on steam", min_delay=0.02, max_delay=0.08, newLine=True,
                               vertical_padding=False)
